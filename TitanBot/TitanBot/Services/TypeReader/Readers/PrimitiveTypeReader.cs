...
namespace TitanBot.Services.TypeReader.Readers
{
    ...
    internal class PrimitiveTypeReader<T> : ITypeReader
    {
        private readonly TryParseDelegate<T> _tryParse;

        public PrimitiveTypeReader()
        {
            _tryParse = PrimitiveTypeReader.Get<T>();
        }

        public ITypeReaderResult Read(IMessageContext context, string text)
        {
            if (_tryParse(text, out T value))
                return TypeReaderResult.FromSuccess(text, value);
    ...
    internal static class PrimitiveTypeReader
    {
        ...
        public static bool TryCreate(Type type, ITypeReaderCollection readerCollection, out IEnumerable<ITypeReader> reader)
        {
            reader = null;
            if (!SupportedTypes.Contains(type))
                return false;
            type = typeof(PrimitiveTypeReader<>).MakeGenericType(type);
            reader = new[] { Activator.CreateInstance(type) as ITypeReader };
            return true;
        }
