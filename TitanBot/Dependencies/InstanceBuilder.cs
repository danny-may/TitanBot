...
{
    partial class DependencyFactory
    {
        class InstanceBuilder : IInstanceBuilder
        {
            ...
            private DependencyFactory Parent { get; }

            private static readonly BindingFlags CtorFlags = BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance;

            public InstanceBuilder(DependencyFactory factory)
            {
                Parent = factory;
                Store = Parent.Stored.ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
                Builders = Parent.Builders.ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
